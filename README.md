...
## 📋 功能特性
- [100%] **活动搜索**: 搜索指定时间范围的行为
    - [100%] 时间范围是可以调节的，比如当日、本月、本年等
- [] 类似toggl的输入框，可输入要执行的行为，并点击开始。
- 行为条目
    - [] 支持条目分类，每个条目可以选择一个行为类别，类别可选择对应的颜色。
    

- ✅ **可视化时间轴**: 24小时横向时间轴，直观显示一天的时间分配
    - ✅ **拖拽创建条目**: 在时间轴上拖拽选择时间范围，快速创建时间条目
- ✅ **自动截图捕获**: 每5分钟自动捕获屏幕截图（后台运行）
- ✅ **窗口活动捕获**: 每分钟记录当前活动窗口标题和进程名
- ✅ **空闲检测**: 检测5分钟以上的空闲时间，支持多种处理方式

- ✅ **数据导出**: 导出所有数据为 JSON 格式
- ✅ **日期导航**: 轻松浏览不同日期的时间记录
- ✅ **截图预览**: 鼠标悬停时间轴查看对应时间点的截图
